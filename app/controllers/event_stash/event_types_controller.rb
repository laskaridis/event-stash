module EventStash
  class EventTypesController < ApplicationController
    ...
    # GET /event_types
    def index
      @event_types = EventStash::EventType.where(account: current_user.email)
    ...
    # GET /event_types/:id
    def show
      @event_type = EventStash::EventType.find(params[:id])
      respond_to do |format|
    ...
    # GET /event_types/new
    def new
      @event_type = EventStash::EventType.new
    end

    # POST /event_types
    def create
      @event_type = EventStash::EventType.new(event_type_params_for_create)
      @event_type.account = current_user.email

      if @event_type.save
        flash[:success] = "Event type #{@event_type.name} has been created successfully"
        redirect_to event_stash_event_types_path
      else
        render :new
      end
    end
    ...
    def event_type_params_for_create
      params.fetch(:event_stash_event_type, {}).permit(
        :name,
        :description,
        :transaction_type,
        :format,
